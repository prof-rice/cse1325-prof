  ...
  try {
    ...
    while(ist) {
        if(std::getline(ist, s)) {
            ...
            std::getline(iss, field, ',') ; temp  = stod(field);
    ...
    while(true) {
        ...
        std::cin >> first; std::cin.ignore();
        if(!std::cin) break;
        
        auto it = temps.find(first);
        if(it == temps.end()) {
            std::cerr << first << " is not in the database!" << std::endl;
            continue;
        }
        
        std::cout << "Ending   date to list (year/month/day): ";
        std::cin >> last; std::cin.ignore();
        if(!std::cin) break;
        
        if(last < first) {
            std::cerr << last << " is earlier than " << first << '!' << std::endl;
            continue;
        }
